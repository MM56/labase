...
class Mobile_Detect
{
     ...
     *
     * @return string One of the self::$operatingSystems.
     */
    public function os()
    {
        $operatingSystems = self::getOperatingSystems();
        foreach($operatingSystems as $operatingSystemName => $operatingSystemRegex) {
            // Escape the special character which is the delimiter.
            $operatingSystemPattern = str_replace('/', '\/', $operatingSystemRegex);

            preg_match('/'.$operatingSystemPattern.'/is', $this->userAgent, $match);
            if(!empty($match)) {
     ...
     *
     * @return string One of the self::$browsers.
     */
    public function browser()
    {
        ...
        foreach($browsers as $browserName => $browserRegex) {
            // Escape the special character which is the delimiter.
            $browserPattern = str_replace('/', '\/', $browserRegex);

            preg_match('/'.$browserPattern.'/is', $this->userAgent, $match);
            if(!empty($match)) {
                return $browserName;
            }
        }
